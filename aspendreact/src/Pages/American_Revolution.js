...
function AmericanRevolutionPack() {
  ...
  useEffect(async () => {
    ...
    await db
      .collection("AmericanRevolutionids")
      .doc("american-ids")
      .get()
      .then((res) => {
        newcollectionIDs = res.data();
      });
  ...
  const BuyPack = async (e) => {
    ...
    if (collectionIDs.length > 0) {
      ...
      const indexOfCollection = Math.floor(
        Math.random() * collectionIDs.length
      );
      ...
      for (var i = 0; i < collectionIDs.length; i++) {
        if (collectionIDs[i] === randomID) {
          ...
          db.collection("AmericanRevolutionids")
            .doc("american-ids")
            .set({ ids: collectionIDs })
            .then(() => {
    ...
    <div className="detail-container w-screen min-h-screen text-[14px] font-medium bg-black text-white tracking-[1.4px] font-Montserrat">
      ...
      <div className="row mt-4 mx-2">
            ...
            <div className="row justify-content-center">
              ...
              <div>
                <p className="col-12 text-center text-md">
                  The revolution that created the United States of America.
                  Taxes, tea and property rights spurred the founding fathers to
                  engage in a struggle for freedom against the rising empire of
                  Britain.
                </p>
              </div>
            </div>
        ...
        <div className="col-md-4" style={maxHeight}>
          <img src={american_revolution} alt="" className="w-full h-auto" />
          <Box m={4} />
          ...
          <h5>
            <strong>Pack's left: {packsLeft}</strong>
          </h5>
